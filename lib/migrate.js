...
function getMigrationsFromDirectory(callback) {
    fs.readdir(_migrationPath, function (err, files) {
        ...
        // TODO to optimize
        files = _.filter(files, function (file) {return /^(m(\d{8}_\d{6})_.*?)\.js$/i.test(file)});
        files = _.map(files, function (file) {return file.slice(0, -3)});
        callback(null, files);
    });
}
...
function getMigrationsFromDb(callback) {
    pool.query(
        'SELECT version ' +
        'FROM ?? ' +
        ...
        function (err, rows) {
            ...
            callback(null, _.pluck(rows, 'version'));
        }
    );
}
...
function getNewMigrations(callback) {
    async.series([
        ...
        callback(null, _.difference(result[0], result[1]));
    });
}
...
function migrateUp (migration, callback) {
    ...
    migrationObject.up(pool, function (err) {
        ...
        addMigrationHistory(migration, function (err) {
            ...
            console.log('*** applied ' + migration + ' (time: ' + time + 'ms)');
    ...
    function fail(err) {
        var time = Math.floor((Date.now() - start) / 1000);
        console.log('*** failed to apply ' + migration + ' (time: ' + time + 'ms)');
