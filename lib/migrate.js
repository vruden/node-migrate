...
function actionNew(limit, callback) {
    limit = limit || 10;
    ...
    getNewMigrations(function (err, migrations) {
        if (err) {
            return callback(err);
        }
        ...
        if (limit && n > limit) {
            ...
            console.log('Showing ' + limit + ' out of ' + n + ' new ' + (n === 1 ? 'migration' : 'migrations') + ':');
        } else {
            console.log('Found ' + n + ' new ' + (n === 1 ? 'migration' : 'migrations') + ' been applied before:');
        }
