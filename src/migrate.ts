...
export class Migration implements IMigration {
    ...
    constructor(config: IMigrateConfig) {
        this.db = new DbHelper(config.dbms, config.connection, config.migrationTableName);
        this.file = new FileHelper(config.migrationPath);
    }
    ...
    up(limit: string, callback): void {
        ...
        this.getNewMigrations(function (err, newMigrations) {
            ...
            prompt.confirm(`Apply the above ${newMigrationNumber === 1 ? 'migration' : 'migrations'}?`, (err, result) => {
                ...
                async.eachSeries(newMigrations,
                    (migration, callback) => {
    ...
    private getNewMigrations(callback: (err: any, migrations?: string[]) => void): void {
